...
int main() {
  ...
  while(true) {
    ...
    // moze se ciklicno mijenjati placeholder karakter
    // npr. u jednoj rundi ih stavljam na x i ne diram x
    // u sljedecoj rundi ih stavljam na y a djelujem na x u valid funkciji
    for(auto& s : matrix)
      for(auto& c : s)
        if(c == 'x')
          c = '.';
